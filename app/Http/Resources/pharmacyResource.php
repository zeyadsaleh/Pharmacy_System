...
class pharmacyResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        return [
            ...
            'action' => '<form method="GET" class="d-inline p-2" action="' . url("/pharmacies", [$this->id, "edit"]) . '"><input type="hidden" name="_token" value="' . csrf_token() . '"><button type="submit" class="d-inline p-2 edit btn btn-primary">Edit</button></form>' . '<button type="button" onclick="deleteAddress('.$this->id.')" class="d-inline p-2 del btn btn-danger">Delete</button>',
        ];
    }
}
